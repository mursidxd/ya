...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    conn.getFile = async (PATH, returnAsFilename) => {
        ...
        if (data && returnAsFilename && !filename) (filename = path.join(__dirname, '../Tempat-Sampah/' + new Date * 1 + '.' + type.ext), await fs.promises.writeFile(filename, data))
        return {
            res,
            filename,
            ...type,
...
END:VCARD`.trim()
    ...
    */
     ...
     */
conn.replyeAR = (jid, text = '', quoted, options) => {
...
conn.sendButtoons = async (jid, text = '', footer = '', buffer, buttons, quoted, options) {
                let type
    ...
    conn.send3Button = async (jid, content, footerText, button1, id1, button2, id2, button3, id3, quoted, options) => {
        ...
        conn.sendBut = async (jid, content, footerText, button1, id1, quoted, options) => {
        const buttons = [
        { buttonId: id1, buttonText: { displayText: button1 }, type: 1 },
        ]
        const buttonMessage = {
    ...
    conn.send2But = async (jid, content, footerText, button1, id1, button2, id2, quoted, options) => {
        const buttons = [
        { buttonId: id1, buttonText: { displayText: button1 }, type: 1 },
        { buttonId: id2, buttonText: { displayText: button2 }, type: 1 }
        ]
